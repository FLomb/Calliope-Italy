...
def power_plot(model_test, start, stop):
    ...
    for zone in locs_dict.keys():
        ...
        bioenergy_dict[zone] = model_test.get_formatted_array('carrier_prod').loc[{'techs':['biomass_wood','biofuel','biogas','wte'],'carriers':'electricity','locs':locs_dict[zone]}].sum('techs').sum('locs').to_pandas().T
        ...
        import_dict[zone] = model_test.get_formatted_array('carrier_prod').loc[{'techs':'el_import','carriers':'electricity','locs':locs_dict[zone]}].sum('locs').to_pandas().T
        ...
        export_dict[zone] = model_test.get_formatted_array('carrier_con').loc[{'techs': ['el_export'],'carriers':'electricity','locs':locs_dict[zone]}].sum('locs').sum('techs').to_pandas().T
    ...
    for zone in locs_dict.keys():
        ...
        ax_dict[zone].set_title("NORD", weight='bold')
        ...
        ax_dict[zone].plot(bio_plot[zone][day:end].index,bio_plot[zone][day:end].values,'#0E5801', alpha=0.2)
        ...
        ax_dict[zone].fill_between(geo_plot[zone][day:end].index,ror_plot[zone][day:end].values,win_plot[zone][day:end].values,facecolor = '#00E71F', alpha = 0.6, label = 'Wind')
        ...
        ax_dict[zone].fill_between(geo_plot[zone][day:end].index,hyd_plot[zone][day:end].values,bio_plot[zone][day:end].values,facecolor = '#0E5801', alpha = 0.6, label = 'Bioenergy')
