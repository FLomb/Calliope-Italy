...
def cap_loc_score_simple(model_new,model_ref,techs=['wind_new','pv_farm_new','pv_rooftop_new','battery','inter_zonal_new:NORD','inter_zonal_new:CNOR','inter_zonal_new:CSUD','inter_zonal_new:SUD','inter_zonal_new:SARD','inter_zonal_new:SICI']):
...
def cap_loc_score_pure(model_new,model_ref,techs=['wind_new','pv_farm_new','pv_rooftop_new','battery','inter_zonal_new:NORD','inter_zonal_new:CNOR','inter_zonal_new:CSUD','inter_zonal_new:SUD','inter_zonal_new:SARD','inter_zonal_new:SICI']):
    cap_per_loc_new = model_new.get_formatted_array('energy_cap').loc[{'techs':techs}].to_pandas()
    cap_per_loc_ref = model_ref.get_formatted_array('energy_cap').loc[{'techs':techs}].to_pandas()
    diff = cap_per_loc_new - cap_per_loc_ref
    diff[diff==0] = 0
...
def curt_exp_vresshare(model):
    ...
    vres_prod = model.get_formatted_array('carrier_prod').loc[{'techs':['pv_farm','pv_rooftop','wind','pv_farm_new','pv_rooftop_new','wind_new']}].sum(['locs','timesteps']).to_pandas().T
    tot_prod = model.get_formatted_array('carrier_prod').loc[{'techs':['wind','pv_farm','pv_rooftop','wind_new','pv_farm_new','pv_rooftop_new','ccgt','coal','coal_usc','oil_&_other','hydro_ror','hydro_dam','biomass_wood','biogas','biofuel','wte','geothermal','el_import']}].sum(['locs','timesteps']).to_pandas().T.sum()
...
def vres_shannon_index(model):
    ...
    wind_prod = model.get_formatted_array('carrier_prod').loc[{'techs':['wind','wind_new']}].sum(['techs','timesteps']).to_pandas().T
...
def vres_new_shannon_index(model,model0):
    ...
    wind_prod = model.get_formatted_array('carrier_prod').loc[{'techs':'wind_new'}].sum('timesteps').to_pandas().T
    tot_vres_prod = pv_prod + wind_prod 
    share_pv = pv_prod / tot_vres_prod.sum()
    share_wind = wind_prod / tot_vres_prod.sum()
    shannon_index = pd.DataFrame(index=tot_vres_prod.index, columns=['Shannon_index'])
...
techs=['inter_zonal_new:NORD','inter_zonal_new:CNOR','inter_zonal_new:CSUD','inter_zonal_new:SUD','inter_zonal_new:SARD','inter_zonal_new:SICI',
       'wind_new','pv_farm_new','pv_rooftop_new','phs','phs_new','wind','pv_farm','pv_rooftop','ccgt','coal','coal_usc','oil_&_other','battery',
...
cap_loc_score_1 = cap_loc_score(model_1,model_0)
...
for m in range(len(nos_dict)):
    ...
    cap_per_loc_vres_dict[m] = cap_loc_calc(nos_dict[m], i=['wind_new','pv_farm_new','pv_rooftop_new','phs_new','battery'])
...
frequency_vres_dict = {}
for t in ['wind_new','pv_new','phs_new','battery']:
...
cap_per_loc_vres_min = cap_loc_calc_min(nos_dict[0], i=['wind_new','pv_farm_new','pv_rooftop_new','phs_new','battery'])
...
for t in ['wind_new','pv_new','battery']:
...
summary_table = pd.DataFrame(index=range(len(nos_dict)),columns=['wind_cap', 'pv_cap', 'tot_vres_cap', 'fossil_dispatchable_cap', 'battery_cap', 'phs_cap', 'transmission_cap', 'vres_prod_share', 'vres_prod_curtailment_fraction', 'export_import_ratio', 'locs_new_vres', 'vres_diversity', 'vres_new_diversity', 'cost', 'co2'])
for m in range(len(nos_dict)):
        ...
        summary_table['tot_vres_cap'].loc[m] =  summary_table['wind_cap'].loc[m] + summary_table['pv_cap'].loc[m]
        ...
        summary_table['locs_new_vres'].loc[m] = (cap_per_loc_vres_dict[m] - cap_per_loc_vres_dict[0])[['wind_new','pv_new']].astype(bool).sum(axis=0).sum()
...
summary_table.to_csv('H:/Lombardi/ETH - Calliope/v.0.16 - updated costs and potentials/Pics/%s/summary_table.csv' %config_name)

n = config_name
os.chdir('H:/Lombardi/ETH - Calliope/v.0.16 - updated costs and potentials/Pics/%s' %n)
summary_table = pd.read_csv('summary_table.csv')
summary_table = summary_table.drop(['Unnamed: 0'], axis=1)

largest_labels = ['wind_cap', 'pv_cap', 'tot_vres_cap', 'battery_cap', 'transmission_cap', 'vres_prod_share', 'vres_prod_curtailment_fraction', 'export_import_ratio', 'locs_new_vres', 'vres_diversity', 'vres_new_diversity']
smallest_labels = ['fossil_dispatchable_cap']
best_dict = {}

for l in largest_labels:
    best_dict[l] = summary_table[l].nlargest(n=2).index.values
for s in smallest_labels:
    best_dict[s] = summary_table[s].nsmallest(n=2).index.values
...
fig, ((ax1, ax2, ax3, ax4, ax17, ax18),(ax5, ax6, ax7, ax8, ax9, ax16), (ax10, ax11, ax12, ax13, ax14, ax15)) = plt.subplots(3,6, sharex='col', sharey='row', gridspec_kw = {'height_ratios':[1,1,1], 'wspace':0.2, 'hspace':0.25}, figsize=(20,10))
...
ax_dict = {'wind_new': ax1,
             ...
             'phs_new': ax17,
             'ccgt': ax5,  
             'coal': ax7, 'coal_usc': ax8,
             'oil_&_other': ax6,
             'inter_zonal_new:NORD': ax10,
             'inter_zonal_new:CNOR': ax11, 
             'inter_zonal_new:CSUD': ax12, 
             'inter_zonal_new:SUD': ax13, 
             'inter_zonal_new:SARD': ax14, 
             'inter_zonal_new:SICI': ax15}
cmap_dict = {'wind_new': 'Greens',
             'pv_farm_new': 'OrRd', 
             'pv_rooftop_new': 'OrRd',
             'ccgt': 'PRGn_r',  
             'coal': 'PRGn_r', 'coal_usc': 'PRGn_r',
             'oil_&_other': 'PRGn_r',
             'battery': 'Blues', 
             'phs_new': 'Blues',
...
techs_heatmap=['inter_zonal_new:NORD','inter_zonal_new:CNOR','inter_zonal_new:CSUD','inter_zonal_new:SUD','inter_zonal_new:SARD','inter_zonal_new:SICI',
       'wind_new','pv_farm_new','pv_rooftop_new','phs_new','ccgt','coal','coal_usc','oil_&_other','battery']

k = 0
for t in techs_heatmap:
    k += 1
    if np.isin(k,[1,2,3,6,8,14]):
...
os.chdir('D:/OneDrive - Politecnico di Milano/ETH - Calliope/v.0.14 - refined transmission cost and no new_ccgt')
...
for nos in indexes_list:
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(1,3, sharey='row', gridspec_kw = {'height_ratios':[1], 'wspace':0.2, 'hspace':0.25}, figsize=(12,6))
    ...
    merged_wind.plot(column=nos_string, cmap=greens, vmax=vmax, linewidth=0.8, ax=ax2, edgecolor='0.6')
    merged_bat.plot(column=nos_string, cmap=blues, vmax=5, linewidth=0.8, ax=ax3, edgecolor='0.6')
    ...
    sm_bat = plt.cm.ScalarMappable(cmap=blues, norm=plt.Normalize(vmin=vmin, vmax=5))
    ...
    fig.tight_layout()
    plt.savefig('D:/OneDrive - Politecnico di Milano/ETH - Calliope/v.0.14 - refined transmission cost and no new_ccgt/Pics/%s/nos_%s_map.png' %(config_name,nos))
    ...
    plt.savefig('D:/OneDrive - Politecnico di Milano/ETH - Calliope/v.0.14 - refined transmission cost and no new_ccgt/Pics/%s/nos_%s_suppl.png' %(config_name,nos))
