...
from nos_utils import cap_loc_score_smart, cap_loc_calc
import numpy as np
import pandas as pd
import os
...
#%% 
'''
Initialising NOS stuff and post-processing functions
'''
...
def regionwide_simple_score(cumulated_simple_score):
    ...
    new_score.loc['NORD'] = cumulated_simple_score.loc[['NORD','R1','R2','R3','R4','R5','R6','R7','R8','AT','CH','FR','SI']].sum().rename('NORD') 
    ...
    new_score.loc['SUD'] = cumulated_simple_score.loc[['SUD','R15','R16','R17','R18','GR']].sum().rename('SUD')
...
def curt_exp_vresshare(model):
    ...
    tot_prod = model.get_formatted_array('carrier_prod').loc[{'techs':['wind_offshore','wind','pv_farm','pv_rooftop','wind_new','pv_farm_new','pv_rooftop_new','ccgt','coal','coal_usc','oil_&_other','hydro_ror','hydro_dam','biomass_wood','biogas','biogas_new', 'biofuel','wte','geothermal','el_import']}].sum(['locs','timesteps']).to_pandas().T.sum()
...
techs = ['inter_zonal_new:FR','inter_zonal_new:AT','inter_zonal_new:CH','inter_zonal_new:SI','inter_zonal_new:GR',
...
#model_1 = calliope.read_netcdf('results_nos_51.nc')
...
cap_loc_score_0 = cap_loc_score_smart(model_0,model_0,techs=techs)
#cap_loc_score_1 = cap_loc_score_smart(model_1,model_0,techs=techs)
...
cap_loc_score_dict[0] = cap_loc_score_0
incremental_score[0] = cap_loc_score_dict[0]
cap_per_loc_dict[0] = cap_loc_calc(nos_dict[0], techs=techs)
...
for j in range(1,(n+1)):
    try:
        ...
        break
    cap_loc_score_dict[j] = cap_loc_score_smart(nos_dict[j],model_0,techs=techs)
    incremental_score[j] = cap_loc_score_dict[j].add(incremental_score[j-1])
    cap_per_loc_dict[j] = cap_loc_calc(nos_dict[j],techs=techs)
...
#    cap_loc_score_supersimple_dict_0[m] = cap_loc_score_simple(nos_dict[m],nos_dict[0])
    cap_per_loc_dict_regionwide[m] = regionwide_simple_score(cap_loc_calc(nos_dict[m], techs=techs))
    cap_per_loc_vres_dict[m] = cap_loc_calc(nos_dict[m], techs=['wind_offshore','wind_new','pv_farm_new','pv_rooftop_new','phs_new','battery'])
...
frequency_dict = {}
cap_per_loc_min = regionwide_simple_score(cap_loc_calc_min(nos_dict[0], techs=techs))
for t in techs:
    frequency_dict[t] = (cap_per_loc_dict_regionwide[0][t]-cap_per_loc_min[t]).to_frame(name='0')
for m in range(1,len(nos_dict)):
    for t in techs:
        frequency_dict[t] = frequency_dict[t].join((cap_per_loc_dict_regionwide[m][t]-cap_per_loc_min[t]).to_frame(name='%d' %m))
...
cap_per_loc_vres_min = cap_loc_calc_min(nos_dict[0], techs=['wind_offshore','wind_new','pv_farm_new','pv_rooftop_new','phs_new','battery'])
...
summary_table = pd.DataFrame(index=range(len(nos_dict)),columns=['wind_cap', 'wind_offshore_cap', 'pv_cap', 'tot_vres_cap', 'battery_cap', 'phs_cap', 'transmission_cap', 'eu_transmission_cap', 'eu_transmission_prod', 'new_bio_cap', 'new_bio_prod', 'vres_prod_share', 'vres_prod_curtailment_fraction', 'export_import_ratio', 'locs_new_vres', 'vres_diversity', 'vres_new_diversity', 'cost', 'co2'])
for m in range(len(nos_dict)):
        ...
        summary_table['new_bio_cap'].loc[m] = float(cap_per_loc_dict[m][['biogas_new']].sum(axis=1).sum(axis=0))/1e6
        summary_table['new_bio_prod'].loc[m] = float(nos_dict[m].get_formatted_array('carrier_prod').loc[{'techs':'biogas_new', 'carriers':'electricity'}].sum('locs').sum('timesteps'))/1e6
        ...
        summary_table['eu_transmission_cap'].loc[m] = float(cap_per_loc_dict[m][['inter_zonal_new:FR','inter_zonal_new:AT','inter_zonal_new:CH','inter_zonal_new:SI','inter_zonal_new:GR']].sum(axis=1).sum(axis=0))/1e6
        summary_table['eu_transmission_prod'].loc[m] = float(nos_dict[m].get_formatted_array('carrier_prod').loc[{'techs':'el_import', 'carriers':'electricity'}].sum('locs').sum('timesteps'))/1e6
        ...
        summary_table['locs_new_vres'].loc[m] = (cap_per_loc_dict[m] - cap_per_loc_dict[0])[['wind_offshore','wind_new','pv_farm_new','pv_rooftop_new']].fillna(0).astype(bool).sum(axis=0).sum()
...
summary_table.to_csv(r'D:\OneDrive - Politecnico di Milano\ETH - Calliope\v.0.24\Pics\%s\summary_table.csv' %config_name)

n = config_name
os.chdir(r'D:\OneDrive - Politecnico di Milano\ETH - Calliope\v.0.24\Pics\%s' %n)
...
smallest_labels = ['new_bio_cap','eu_transmission_cap','new_bio_prod','eu_transmission_prod']
best_dict = {}
...
ax_dict = {'wind_new': ax1,
           ...
           'phs_new': ax6,
           'biogas_new': ax7,  
           'inter_zonal_new:FR': ax8, 
           'inter_zonal_new:AT': ax9,
           'inter_zonal_new:CH': ax10,
           'inter_zonal_new:SI': ax17,
           'inter_zonal_new:GR': ax18,
...
cmap_dict = {'wind_new': 'Greens',
             ...
             'pv_rooftop_new': 'OrRd',
             'biogas_new': 'Greens',  
             'inter_zonal_new:FR': 'Purples', 
             'inter_zonal_new:AT': 'Purples',
             'inter_zonal_new:CH': 'Purples',
             'inter_zonal_new:SI': 'Purples',
             'inter_zonal_new:GR': 'Purples',
             'battery': 'Blues', 
             'phs_new': 'Blues',
...
vmax = {'wind_new': 20,
        ...
        'pv_rooftop_new': 20,
        'biogas_new': 4,  
        'inter_zonal_new:FR': 1, 
        'inter_zonal_new:AT': 1,
        'inter_zonal_new:CH': 1,
        'inter_zonal_new:SI': 1,
        'inter_zonal_new:GR': 1,
        'battery': 10, 
        'phs_new': 1,
...
#ax17.axis('off')
#ax18.axis('off')
fig.savefig(r'D:\OneDrive - Politecnico di Milano\ETH - Calliope\v.0.24\Pics\%s\heatmaps_plot.png' %config_name)
     

#%% Choropleth map

os.chdir(r'D:\OneDrive - Politecnico di Milano\ETH - Calliope\v.0.24')
...
#    fig.tight_layout()
#    plt.savefig(r'D:\OneDrive - Politecnico di Milano\ETH - Calliope\v.0.24\Pics\test' %(config_name,nos))
#     
#    
...
#    
    ...
    plt.savefig(r'D:\OneDrive - Politecnico di Milano\ETH - Calliope\v.0.24\Pics\%s\nos_%s_zdisp.png' %(config_name,nos))
