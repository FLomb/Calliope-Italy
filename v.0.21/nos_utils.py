...
def cap_loc_score_0(model_new,model_ref,techs=['wind','pv_farm','pv_rooftop','ccgt','coal','coal_usc','oil_&_other','battery','inter_zonal:NORD','inter_zonal:CNOR','inter_zonal:CSUD','inter_zonal:SUD','inter_zonal:SARD','inter_zonal:SICI']):
    cap_per_loc_new = model_new.get_formatted_array('energy_cap').loc[{'techs':techs}].to_pandas()
    cap_per_loc_ref = model_ref.get_formatted_array('energy_cap_min').loc[{'techs':techs}].to_pandas()
    diff = cap_per_loc_new - cap_per_loc_ref
    diff[diff>0] = 1
    diff[diff==0] = 0
    diff[diff<0] = 0
